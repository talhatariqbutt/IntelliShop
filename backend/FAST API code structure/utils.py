...
#     with torch.no_grad():
#         distances, indices = index.search(np.expand_dims(query_features, axis=0).astype(np.float32), 20)
...
#         'Product_Description':df['Product_Description'].iloc[indices[0]],
#     })
    
#     dino_df = dino_df.drop_duplicates(subset='Product_Id')

#     # Ensure we keep up to the first k unique products
#     dino_df = dino_df.head(k)

#     # Create a dictionary to store URLs for each product_id
#     product_urls = {}

#     # Iterate over each unique product_id in dino_df to gather all URLs per product
#     for product_id in dino_df['Product_Id']:
